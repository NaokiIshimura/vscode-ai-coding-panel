...
### Fixed
- **Runボタンでエディタの内容がシェルに解釈される問題を修正**: 1.2.4でRunボタンが開いているファイルの内容を送信するようになった結果、Markdownのコードフェンスやインラインコードを含むファイルでは本文の一部がシェルコマンドとして実行され、`claude` には冒頭しか渡らない状態でした
  - `_escapeShellArgument()` が値をシングルクォートで囲む一方、コマンドテンプレートはプレースホルダをダブルクォートで囲むため、`claude "'# task ...'"` という形になっていました。シングルクォートはただの文字となり、`` ` `` `$` `\` が展開対象のまま残るため、インラインコードの `` `claude attach xxx` `` がコマンド置換として実行され、コードフェンスは空のバッククォート対となって `command not found` を起こしていました
  - 値を `"'値'"` の形で囲むようにしました。テンプレートが開いたダブルクォートを一度閉じ、値をシングルクォートで保護してから再度開きます。シェルは隣接する引用符を連結するため、結果は1つの引数のまま扱われます
  - コマンドの送信先は対話シェルであり、ダブルクォート内の `!` はヒストリ展開として解釈されます（`event not found`）。これを防げるのはシングルクォートのみのため、テンプレートではなくエスケープ処理を変更しています
- **Plan / Specがファイルパスを余計な引用符なしで渡すように修正**: `${filePath}` にも同じエスケープを使うため、プロンプトがパスを引用符で囲んだ状態（`Review the file at '.claude/plans/x.md' and ...`）ではなく `Review the file at .claude/plans/x.md and ...` となります。`` ` `` や `$` を含むパスも展開されなくなりました

### Changed
- **コマンドテンプレートのプレースホルダはダブルクォートで囲む必要があります**: 既定値・ドキュメントの例はすべてこの形のため、プレースホルダを囲まない形にカスタマイズしている場合を除き対応は不要です。その場合は `unmatched '` で失敗します。設定の説明・README 2種・`docs/editor-view.md` に明記しました
