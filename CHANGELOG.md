...
### Changed
- **Command Prefix Default**: Added `--enable-auto-mode` option to the default `commandPrefix` setting
  - Default value changed from `claude` to `claude --enable-auto-mode`
  - Auto mode is now enabled by default for Run/Plan/Spec commands
  - Users can revert to the previous behavior by changing `commandPrefix` to `claude` in settings

## [1.0.16] - 2026-03-08

### Changed
- **Prompt/Spec/Task Templates**: Updated metadata section in all three built-in template files (`templates/prompt.md`, `templates/spec.md`, `templates/task.md`)
  - Removed `working dir: {{dirpath}}` line
  - Added `memory  : {{dirpath}}` line to indicate where session memory is stored
  - Renamed `prompt file: {{filename}}` to `prompt  : {{filename}}`
...
### Fixed
- **Plans View Auto-Refresh**: Fixed issue where files added to the currently open directory in Plans View were not displayed until manual refresh
  - Changed FileWatcher notification handler to clear all item cache instead of partial path-based cache clearing, preventing missed updates due to path normalization differences or debounce event consolidation
  - Added `refresh()` call when Plans View becomes visible again, ensuring any file changes missed while the view was hidden are reflected immediately
...
### Added
- **Terminal Shortcut: claude update**: Added `claude update` button to Terminal View's shortcut bar when Claude Code is not running
  - Positioned as the rightmost button (before the toggle button)
  - Executes `claude update` command to update Claude CLI directly from the terminal shortcuts
  - Does not start an interactive Claude Code session (`startsClaudeCode: false`)
...
### Changed
- **Test Code Improvements**: Translated all Japanese comments in test files to English for consistency
...
### Added
- **Model Switching Shortcuts**: Added `/model sonnet` and `/model opus` shortcut buttons to Terminal View
  - Buttons appear in the Claude Code running shortcut bar
  - Placed to the left of `/compact` for quick model changes
  - Enables rapid switching between Sonnet and Opus models during Claude Code sessions
