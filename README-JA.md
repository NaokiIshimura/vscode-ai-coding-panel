...
| **コンテキスト対応ショートカット** | 状態に応じて変化するスマートボタン：<br>- 未起動時: `claude`, `claude -c`, `claude -r`, `claude --permission-mode auto`, `claude update`<br>- 起動中: `/model sonnet`, `/model opus`, `/compact`, `/clear`<br>Claude Codeセッション中に素早いモデル切り替えと共通コマンドが実行可能 |
