...
| **Context-Aware Shortcuts** | Smart buttons that change based on state:<br>- Not running: `claude`, `claude -c`, `claude -r`, `claude --permission-mode auto`, `claude update`<br>- Running: `/model sonnet`, `/model opus`, `/compact`, `/clear`<br>Enables quick model switching and common commands during Claude Code sessions |
